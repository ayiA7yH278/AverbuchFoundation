...
export default function PrescottSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `/${slug}/services`, label: 'Practice Areas', pg: 'services' },
    ...
    <div style={{ fontFamily: fonts.body, color: colors.foreground }}>
      ...
      <nav className="sticky top-0 z-50 px-6 py-4" style={{ backgroundColor: colors.primary }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/" className="text-xs opacity-40 hover:opacity-100" style={{ color: colors.primaryForeground }}><ArrowLeft className="w-3.5 h-3.5" /></Link>
            <Link to={`/${slug}`} className="text-xl font-bold" style={{ fontFamily: fonts.heading, color: colors.secondary }}>{b.name}</Link>
          </div>
        ...
        <>
          ...
          <section className="min-h-[75vh] flex items-center justify-center text-center px-6" style={{ backgroundColor: colors.primary, color: colors.primaryForeground }}>
            <div className="max-w-3xl">
              ...
              <Link to={`/${slug}/contact`} className="inline-flex items-center gap-2 px-8 py-3.5 text-sm font-medium border-2 transition-colors hover:bg-white/10" style={{ borderColor: colors.secondary, color: colors.secondary }}>
          ...
          <section className="py-24 px-6" style={{ backgroundColor: colors.background }}>
            <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
              ...
              <div>
                ...
                <Link to={`/${slug}/about`} className="text-sm font-medium inline-flex items-center gap-1 hover:gap-2 transition-all" style={{ color: colors.secondary }}>Read More <ArrowRight className="w-3.5 h-3.5" /></Link>
              </div>
            </div>
          </section>
      ...
      <footer className="py-10 px-6" style={{ backgroundColor: colors.primary, color: colors.primaryForeground }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-sm opacity-60">{b.name} © 2026 Enterprise</p>
          <div className="flex gap-6">{links.map(l => <Link key={l.pg} to={l.path} className="text-sm opacity-40 hover:opacity-100">{l.label}</Link>)}</div>
        </div>
      </footer>
    </div>
  );
