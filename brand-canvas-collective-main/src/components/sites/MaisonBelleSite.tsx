...
export default function MaisonBelleSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `/${slug}/contact`, label: 'Book', pg: 'contact' },
  ];
    ...
    <div style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.foreground }}>
      ...
      <nav className="sticky top-0 z-50 px-6 py-4" style={{ backgroundColor: `${colors.background}F0`, backdropFilter: 'blur(12px)' }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/" className="text-xs opacity-40 hover:opacity-100"><ArrowLeft className="w-3 h-3" /></Link>
            <Link to={`/${slug}`} className="text-2xl font-light italic" style={{ fontFamily: fonts.heading, color: colors.primary }}>{b.name}</Link>
          </div>
          ...
          <section className="min-h-[85vh] flex items-center py-20 px-6" style={{ backgroundColor: colors.background }}>
            <div className="max-w-7xl mx-auto w-full grid md:grid-cols-5 items-center gap-0">
              ...
              <div className="md:col-span-2 md:-ml-16 relative z-10 p-8 md:p-12 rounded-2xl shadow-xl mt-[-3rem] md:mt-0" style={{ backgroundColor: colors.background }}>
                ...
                <Link to={`/${slug}/contact`} className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-medium border transition-colors" style={{ borderColor: colors.primary, color: colors.primary }}>
        ...
        <section className="py-20 px-6">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="text-center mt-12">
              <Link to={`/${slug}/contact`} className="inline-flex items-center gap-2 px-6 py-3 rounded-full border text-sm" style={{ borderColor: colors.primary, color: colors.primary }}>{hero.cta} <ArrowRight className="w-4 h-4" /></Link>
            </div>
          </div>
        </section>
      )}
      ...
      <footer className="py-8 px-6" style={{ backgroundColor: colors.primary, color: colors.primaryForeground }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-sm opacity-70 italic" style={{ fontFamily: fonts.heading }}>{b.name} © 2026 Enterprise</p>
          <div className="flex gap-6">{links.map(l => <Link key={l.pg} to={l.path} className="text-xs opacity-50 hover:opacity-100 uppercase tracking-wider">{l.label}</Link>)}</div>
        </div>
      </footer>
    </div>
  );
