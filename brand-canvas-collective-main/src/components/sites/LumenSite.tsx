...
export default function LumenSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `/${slug}/services`, label: 'Work', pg: 'services' },
    { path: `/${slug}/contact`, label: 'Inquire', pg: 'contact' },
  ];
  ...
  return (
    <div style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.foreground }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-5 mix-blend-difference">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/" className="text-xs text-white opacity-40 hover:opacity-100"><ArrowLeft className="w-3 h-3" /></Link>
            <Link to={`/${slug}`} className="text-sm font-medium tracking-[0.2em] uppercase text-white">{b.name}</Link>
          </div>
      ...
      <footer className="py-8 px-6 border-t" style={{ borderColor: `${colors.muted}15` }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-xs tracking-[0.15em] uppercase" style={{ color: colors.muted }}>{b.name} © 2026 Enterprise</p>
          <div className="flex gap-6">{links.map(l => <Link key={l.pg} to={l.path} className="text-xs tracking-widest uppercase" style={{ color: colors.muted }}>{l.label}</Link>)}</div>
        </div>
      </footer>
    </div>
  );
