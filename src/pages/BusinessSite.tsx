...
export default function BusinessSite() {
  ...
  if (!business) return <Navigate to="/gallery" replace />;

  const SiteComponent = siteMap[business.slug];
  if (!SiteComponent) return <Navigate to="/gallery" replace />;

  const pathParts = location.pathname.split('/').filter(Boolean);
  const page = pathParts.length > 2 ? pathParts[pathParts.length - 1] : 'home';

  return <SiteComponent business={business} page={page} />;
}
