...
export default function ForgeSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `${base}/services`, label: 'PROGRAMS', pg: 'services' },
    { path: `${base}/contact`, label: 'JOIN', pg: 'contact' },
  ];
  ...
  return (
    <div style={{ fontFamily: fonts.body, color: '#FFFFFF' }}>
      ...
      <nav className="sticky top-0 z-50 px-6 py-4" style={{ backgroundColor: '#0A0A0A' }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/gallery" className="text-xs opacity-30 hover:opacity-100 text-white"><ArrowLeft className="w-3.5 h-3.5" /></Link>
            <Link to={base} className="text-xl font-black uppercase tracking-wider" style={{ fontFamily: fonts.heading, color: colors.primary }}>{b.name}</Link>
          </div>
        ...
        <>
          ...
          <section className="min-h-screen flex items-end pb-16 md:pb-24 px-6 relative" style={{ backgroundColor: '#0A0A0A' }}>
            ...
            <div className="relative z-10 max-w-4xl">
              ...
              <Link to={`${base}/contact`} className="inline-flex items-center gap-3 px-8 py-4 text-sm font-bold uppercase tracking-widest" style={{ backgroundColor: colors.primary, color: '#FFFFFF' }}>
      ...
      <BrandSiteFooter name={b.name} backgroundColor="#0A0A0A" />
    </div>
  );
}
