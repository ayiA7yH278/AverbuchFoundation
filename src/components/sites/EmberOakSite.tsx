...
export default function EmberOakSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `${base}/services`, label: 'Menu', pg: 'services' },
    { path: `${base}/contact`, label: 'Visit', pg: 'contact' },
  ];
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-5">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/gallery" className="text-xs opacity-50 hover:opacity-100 transition-opacity flex items-center gap-1" style={{ color: page === 'home' ? colors.primaryForeground : colors.foreground }}>
              <ArrowLeft className="w-3 h-3" /> Gallery
            </Link>
            <Link to={base} className="text-xl font-bold" style={{ fontFamily: fonts.heading, color: page === 'home' ? colors.primaryForeground : colors.primary }}>{b.name}</Link>
          </div>
