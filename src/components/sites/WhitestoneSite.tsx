...
export default function WhitestoneSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `${base}/about`, label: 'Studio', pg: 'about' },
      ...
      <nav className="sticky top-0 z-50 px-6 py-4 border-b-2" style={{ backgroundColor: colors.background, borderColor: colors.foreground }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/gallery" className="text-xs opacity-30 hover:opacity-100"><ArrowLeft className="w-3.5 h-3.5" /></Link>
            <Link to={base} className="text-lg font-black uppercase tracking-[0.15em]" style={{ fontFamily: fonts.heading }}>{b.name}</Link>
          </div>
          ...
          <section className="min-h-[80vh] grid md:grid-cols-2">
            <div className="flex items-center p-10 md:p-16 lg:p-24 border-r-2" style={{ borderColor: colors.foreground }}>
              <div>
                ...
                <Link to={`${base}/services`} className="inline-block px-6 py-3 text-xs uppercase tracking-widest font-bold border-2 hover:bg-black hover:text-white transition-colors" style={{ borderColor: colors.foreground }}>{hero.cta}</Link>
              </div>
            </div>
