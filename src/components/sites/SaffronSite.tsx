...
export default function SaffronSite({ business: b, page }: Props) {
  ...
  const links = [
    ...
    { path: `${base}/about`, label: 'Story', pg: 'about' },
    { path: `${base}/services`, label: 'Dining', pg: 'services' },
    { path: `${base}/contact`, label: 'Reserve', pg: 'contact' },
  ];
    ...
    <div style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.foreground }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-5">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link to="/gallery" className="text-xs opacity-40 hover:opacity-100" style={{ color: page === 'home' ? '#FFF8F0' : colors.foreground }}><ArrowLeft className="w-3.5 h-3.5" /></Link>
          <Link to={base} className="text-2xl font-bold tracking-wider" style={{ fontFamily: fonts.heading, color: page === 'home' ? '#FFF8F0' : colors.primary }}>{b.name}</Link>
        ...
        <>
          ...
          <section className="relative h-screen flex items-center justify-center">
            ...
            <div className="relative text-center px-6" style={{ color: colors.primaryForeground }}>
              ...
              <Link to={`${base}/contact`} className="inline-block px-8 py-3.5 text-sm uppercase tracking-[0.2em] font-medium border transition-colors hover:bg-white/10" style={{ borderColor: colors.secondary, color: colors.secondary }}>
                {hero.cta}
              </Link>
            </div>
          </section>
